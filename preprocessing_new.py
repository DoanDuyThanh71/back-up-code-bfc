...
import joblib
# Import the NEW 33-column merge logic
from merge_files_33 import merge_ingredients, convert_x2x, str_to_bool
import os

newFileName = "final 8 mã số thuế"
...
# Market Classification - using simple placeholders since we cannot be sure dim file fits perfectly
# Actually we can try to reuse the code
if company := 'Công_ty_nhập': # Assuming export_data=False
    comp_nice_name = company.lower().replace('_', " ")
    parent_company = 'Công ty nhập gộp'
    
    # Try reading the dim file if exists, else skip
    dim_file = '../../Data/Data for fill data/Updated dim_company_marketType.xlsx' 
    # Use empty if not found, we don't want to break
...
columns_to_select = [
    ...
    'Nước_nhập_khẩu', 'HScode', 'Mô_tả_sản_phẩm', 'Brand', 'Thương hiệu gộp', 'Hợp_lệ', 
...
new_column_names = [
    ...
    'Quốc gia nhập khẩu', 'HScode', 'Mô tả sản phẩm', 'Thương hiệu', 'Thương hiệu gộp', 'Hợp lệ',    
