...
def merge_ingredients(downloadPath, fileExtension='.xlsx', export_data=False):
    ...
    for currentFile in currentFiles:
        # Filter for the specific export files based on user context (or process all)
        # Assuming we only want to process the NEW files or all files as export?
        # The user said "Tow file trong thu muc download", implying specific files.
        # But for simplicity, let's process all xlsx files but apply EXPORT mapping.
        ...
        if len(df.columns) == 33:
             # EXPORT MAPPING (Swapped Col 2 and 5)
             # Index 2: 进口商 (Importer) -> Foreign Partner -> 'Nhà_cung_cấp'
             # Index 5: 出口商 (Exporter) -> Local Company (Vietnam) -> 'Công_ty_nhập'
             # Index 4: 目的国 (Dest Country) -> 'Nước_nhập_khẩu'
             
             df.columns = ['Date', 'Mã_tờ_khai', 'Nhà_cung_cấp', # Index 2
            'Địa_chỉ', "Nước_nhập_khẩu", 'Công_ty_nhập', # Index 5
            "Nhà_cung_cấp (TA)", "Mã_số_thuế", "Quốc_gia_xuất_xứ",
            ...
            '海关代码',	"海关代理代码",	"Cảng xuất","起运港代码",	"Cảng nhập", "目的港代码", "运输方式", "承运人","追踪号"]
             ...
             # Note: 'Công_ty_nhập' is used for downstream processing to find Brand.
             # 'Nhà_cung_cấp' is the Foreign Partner.
             
        elif len(df.columns) == 38:
            # Keep original import mapping for 38 columns if any
        ...
        if product_name:
            ...
            with open("merged_chats_export.txt", mode='w', encoding= 'utf-8') as file:
                file.write(", \n".join(merged_chats))
        else:
