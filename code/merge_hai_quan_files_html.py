...
def merge_hai_quan_files_html(folder_path, output_file="merged_hai_quan_data.xlsx"):
    """
    Nối tất cả các file Excel/HTML trong thư mục dữ liệu hải quan
    Loại trừ file 'Form lấy dữ liệu hải quan.xlsx'
    Thêm cột 'Ten_File_Goc' và 'Ten_Sheet_Goc' để nhận biết file và sheet gốc
    """
    ...
    for file_path in excel_files:
        try:
            ...
            # Cách 1: Kiểm tra xem có phải file HTML không
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    first_line = f.read(100)
                    if '<table' in first_line.lower() or '<html' in first_line.lower():
                        print(f"  - Phát hiện file HTML, đọc bằng pandas...")
                        # Đọc file HTML
                        df_list = pd.read_html(file_path, encoding='utf-8')
                        
                        if df_list:
                            print(f"    ✓ Tìm thấy {len(df_list)} bảng trong file HTML")
                            
                            for i, df in enumerate(df_list):
                                if not df.empty:
                                    # Thêm cột để nhận biết file và sheet gốc
                                    df['Ten_File_Goc'] = file_name
                                    df['Ten_Sheet_Goc'] = f"Table_{i+1}"
                                    
                                    # Thêm vào danh sách
                                    all_dataframes.append(df)
                                    print(f"      ✓ Bảng {i+1}: {len(df)} dòng")
                                else:
                                    print(f"      - Bảng {i+1}: Rỗng, bỏ qua")
                            
                            success = True
                        else:
                            print(f"    - Không tìm thấy bảng nào trong file HTML")
                            
            except Exception as e:
                print(f"  - Không phải file HTML hoặc lỗi đọc HTML: {str(e)}")
            
            # Cách 2: Thử đọc như file Excel thông thường
            if not success:
                engines_to_try = ['xlrd', 'openpyxl', 'calamine']
                for engine in engines_to_try:
                    try:
                        print(f"  - Thử với engine: {engine}")
                        excel_file = pd.ExcelFile(file_path, engine=engine)
                        sheet_names = excel_file.sheet_names
                        print(f"    ✓ Thành công với {engine}, số sheet: {len(sheet_names)}")
                        ...
                        print(f"    ✗ {engine} không hoạt động: {str(e)}")
                        continue
            
            if not success:
                print(f"✗ Không thể đọc file {os.path.basename(file_path)} với bất kỳ phương pháp nào")
    ...
    if all_dataframes:
        ...
        if len(file_counts) > 10:
            print(f"  ... và {len(file_counts) - 10} file khác")
...
def main():
    ...
    print("=== CHƯƠNG TRÌNH NỐI FILE DỮ LIỆU HẢI QUAN (HTML/EXCEL) ===")
    print(f"Thư mục: {folder_path}")
    print(f"File kết quả: {output_file}")
    print("=" * 65)
    
    # Thực hiện nối file
    result = merge_hai_quan_files_html(folder_path, output_file)
