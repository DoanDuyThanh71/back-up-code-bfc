...
def merge_hai_quan_files_xlwings(folder_path, output_file="merged_hai_quan_data.xlsx"):
    """
    Nối tất cả các file Excel trong thư mục dữ liệu hải quan sử dụng xlwings
    ...
    for file_path in excel_files:
        try:
            ...
            try:
                ...
                print(f"  - Đọc thành công với xlrd, số sheet: {len(sheet_names)}")
                ...
                print(f"  - xlrd không hoạt động: {str(e)}")
            
            # Cách 2: Thử với pandas và engine khác nhau
            if not success:
...
def main():
    ...
    print("=== CHƯƠNG TRÌNH NỐI FILE DỮ LIỆU HẢI QUAN (XLWINGS) ===")
    print(f"Thư mục: {folder_path}")
    print(f"File kết quả: {output_file}")
    print("=" * 60)
    
    # Thực hiện nối file
    result = merge_hai_quan_files_xlwings(folder_path, output_file)
